...
Install_SS_python() {
  ...
  if [ -f ${python_install_dir}/bin/ssserver ]; then
    if [ -e /bin/systemctl ]; then
      /bin/cp ../init.d/xx-python.service /lib/systemd/system/shadowsocks.service
      sed -i "s@/usr/local/python@${python_install_dir}@g" /lib/systemd/system/shadowsocks.service
      systemctl enable shadowsocks
    else
      /bin/cp ../init.d/xx-python-init /etc/init.d/shadowsocks
...
#  src_url=http://mirrors.linuxeye.com/oneinstack/src/libsodium-${libsodium_ver}.tar.gz && Download_src
  src_url= https://github.com/jedisct1/libsodium/releases/download/${libsodium_ver}-RELEASE/libsodium-${libsodium_ver}.gz && Download_src
#  src_url=http://mirrors.linuxeye.com/oneinstack/src/mbedtls-2.28.3-apache.tgz && Download_src
  src_url=https://github.com/ARMmbed/mbedtls/archive/mbedtls-2.28.3.tar.gz && Download_src
  ...
  tar xzf mbedtls-2.28.3.tar.gz
  pushd mbedtls-mbedtls-2.28.3 > /dev/null
  ...
  if [ -f /usr/local/bin/ss-server ]; then
    if [ -e /bin/systemctl ]; then
      /bin/cp ../init.d/xx-libev.service /lib/systemd/system/shadowsocks.service
      systemctl enable shadowsocks
    else
      if [ "${PM}" == 'yum' ]; then
        /bin/cp ../init.d/xx-libev-init-CentOS /etc/init.d/shadowsocks
        chkconfig --add shadowsocks
        chkconfig shadowsocks on
      elif [ "${PM}" == 'apt-get' ]; then
        /bin/cp ../init.d/xx-libev-init-Ubuntu /etc/init.d/shadowsocks
...
# , "plugin":"obfs-server",
# "plugin_opts":"obfs=http;failover=www.bing.com"
EOF
